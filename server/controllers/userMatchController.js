...
const addUserMatch = async (req, res) => {
  try {
    const { user_uuid, points_of_contact } = req.body;
...
const getUserMatches = async (req, res) => {
  try {
    ...
    const userMatches = await User.find(
      ...
      "username gender points_of_contact trip_wishlist"
    )
      .lean()
      .exec();
    ...
    let userMatchesHandles = new Set(
      userMatchesWithGenderPreference.map((u) => u.username)
    );
    // get trips for users with same destination
    const allTrips = await Trip.find({ destination: destination })
      .lean()
      .exec();
    // filter on gender preference users
    const tripsForMatchingUsers = allTrips.filter((trip) =>
      userMatchesHandles.has(trip.username)
    );
    // filter trips on activities
    const tripsWithMatchingActivities = tripsForMatchingUsers.filter((trip) => {
      const entries = Object.entries(trip.activities_preference);
      const intersection = entries.filter(
        (pair) => pair[0] in activities_preference
      );
